...
angle(const Momentum &a, const Momentum &b)
{
  ...
  //return zwerg == 0 ? 0 : acos(a*b/zwerg); //bss: for angle=0deg, this can result in rounding prob! acos(>1) etc.
  double ab_zwerg=a*b/zwerg;
  if (ab_zwerg>=1)
    ab_zwerg=1.0;
  else if (ab_zwerg<=-1.0)
    ab_zwerg=-1.0;
  return zwerg == 0 ? 0 : acos(ab_zwerg);
}
...
std::ostream& operator<<(std::ostream& out, FourVector f) 
{
  out.setf(std::ios::fixed, std::ios::floatfield);
  int p = out.precision(6);
  out << std::setw(12) << f.energy() << std::setw(12) << f[1]
      << std::setw(12) << f[2] << std::setw(12) << f[3]
      << std::setw(12) << f.square() << std::setw(12) << f.Ekin()
      << std::setw(12) << f.momentum();
  out.precision(p);
  out.setf(std::ios::right, std::ios::adjustfield);
