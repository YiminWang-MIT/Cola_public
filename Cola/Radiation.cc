...
double Landau_g(double mean, double sigma, Uniform& rndm)
{
//  Generate a random number following a Landau distribution</b>
//  with average value mean and rms</b>
//  Converted by Rene Brun from CERNLIB routine ranlan(G110)</b>
		 ...
		 Uniform& rndm1, Uniform& rndm2) 
{
  ...
  double dE;
  double emax=p.energy();

   dE = Landau_g(m->dEdx(p, x), zeta, rndm1);
   if  (-dE>emax) dE=-emax;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//     bss 2016 based on Vanderhaeghen PRC 62 (2000) 025501 combined with Maximon Tjon PRC 62 (2000) 054320 [hadron side]     //
//     things are not merged -> longer, but better comparable                                                                 //
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//finite part of the lepton vertex correction, Q^2 >> m_e
//Vanderhaeghen (A67), Bernauer (2.16)
double Radiation_lepton_vertex(double Q2)
{
  double delta = alpha/M_PI * (1.5*log(Q2/m_e_sqr) -2. -0.5*pow(log(Q2/m_e_sqr),2) + M_PI*M_PI/6.);
  return delta;
}
//finite and Delta E' independant part of the real radiation, electron side
//vdh(A65)
double Radiation_lepton_rr(double Q2, double E, double Ep, double theta) 
{
  double delta = alpha/M_PI * (-0.5*pow(log(E/Ep),2) +0.5*pow(log(Q2/m_e_sqr),2) -M_PI*M_PI/3 + Spence(pow(cos(theta/2),2)));
  return delta;
}
//finite part of the photon polarization correction, for Q^2 >> m_l^2, which is not good for l=muon, tau
//Vanderhaeghen (A69), Bernauer (2.15)
double Radiation_vac_l_approx(double Q2)
{
  double delta =  alpha/M_PI * 2. / 3. * (-5./3. + log(Q2/m_e_sqr));
  return delta;
}
//finite part of the photon polarization correction, for Q^2 >> m_l^2, also for l=muon, tau
//Vanderhaeghen (A68), Bernauer (2.14)
double Radiation_vac_l(double Q2, double ml)
{
  double v2 = 1. + 4.*ml*ml / Q2; //Vdh. (11); Bernauer below (2.15)
  double v = sqrt(v2);
  double delta =  alpha/M_PI * 2. / 3. * ((v2 -8./3.)  +   v*(3.-v2)/2. * log((v+1.)/(v-1.)));
  return delta;
}
//finite part of the photon polarization correction, sum of electron, muon, tau loops
double Radiation_vac(double Q2, int mucontribution=1, int taucontribution=0)
{
  double delta = Radiation_vac_l(Q2, m_electron);
  if (mucontribution!=0)
    delta += Radiation_vac_l(Q2, m_mu);
  if (taucontribution!=0)
    delta += Radiation_vac_l(Q2, m_tau);
  return delta;
}
//Z dependant, Delta E' independant part of hadron correction (MaximonTjon2000)
double Radiation_hadron_Z(double Z, double x, double eta)
{
  return 2*alpha*Z/M_PI * (Spence(1.-eta/x)-Spence(1.-1.0/(eta*x)));
}
//Z^2 dependant, Delta E' independant, hadron structure independant part of hadron correction (MaximonTjon2000)
double Radiation_hadron_ZZ(double Z, double M, double E_hadron, double p_hadron, double rho, double x)
{
  double lx=log(x);
  return alpha*Z*Z/M_PI * (1.+E_hadron/p_hadron*(-0.5*lx*lx -lx * (2*log(rho/M)) +lx-Spence(1-1.0/(x*x))+2*Spence(-1.0/x)+M_PI*M_PI/6));
}
///////////////////////////////////////////////////////
///////// main routines using the ones above: /////////
///////////////////////////////////////////////////////
//correction for finite and Delta E' independent lepton and vacuum polarization contributions:
double Radiation_LeptonAndVacuumCorrection(double Q2, double E, double Ep, double theta)
{
  double delta_lepton_vertex = Radiation_lepton_vertex(Q2);
  double delta_lepton_rr     = Radiation_lepton_rr(Q2, E, Ep, theta);
  double delta_vac           = Radiation_vac(Q2, 1, 0);
  //this is the very same as standard vertexCorrection if used with "Radiation_vac(Q2, 0, 0)" (leave out vacuum muon contribution, small)
  //and exp(delta_vac) instead of 1/pow(1-delta_vac/2,2) (minimal difference)
  return exp(delta_lepton_vertex+delta_lepton_rr)/pow(1-delta_vac/2,2); //compare vdh(A72)
}
//correction for finite and Delta E' independent and hadron structure independent hadron contribution:
double Radiation_HadronCorrection(double Z, double M, double E_hadron,double p_hadron, double rho, double x, double eta)
{
  // JCB: "this is the maximon tjon version. marc says: it is better [than mo tsai code]!"
  double deltaZ= Radiation_hadron_Z (Z, x, eta);
  double deltaZZ=Radiation_hadron_ZZ(Z, M, E_hadron, p_hadron,  rho,  x);
  return exp(deltaZ + deltaZZ);
}
