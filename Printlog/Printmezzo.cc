...
//
// $Id: Printmezzo.cc 2545 2014-03-18 14:28:34Z merkel $
//
...
{ argv[0]=av[0];
  ...
  int opt, compress=0;
  while (EOF != (opt = getopt(ac, av, "b:e:f:ho:zp:"))) 
  { switch (opt) 
    { case 'b' : strcpy(begintime = new char[255], optarg);break;
      ...
      case 'p' : strcpy(pattern = new char[255], optarg);
	         if (regcomp(&reexp, pattern, REG_EXTENDED|REG_NOSUB))
      ...
      case 'h' : puts("Usage: Printlog++ [options]");
                 ...
                 puts("-p                regular expression for alias string");
...
void split(FILE *fp, char *string, int N, char term='|') {
  for (int i=0; i<N; i++) {
    if (feof(fp)) { string[i]='\0'; return;}
    string[i] = fgetc(fp);
    if (string[i] == term) { string[i]='\0'; return;} 
  }
  string[N-1] = '\0';
  fprintf(stderr,"String to long %s\n",string);
  return;
}
...
int work (wxApp *app)
{
  ...
  if (!feof(fp))
  { char *dev, linedate[N], state[N],value[N];
    split(fp, device, N);
    split(fp, linedate,N);
    split(fp, state, N);
    split(fp, value, N, '\n');
    
    for (int i=0;device[i];i++) if (device[i]=='.') device[i] = '/';
    for (int i=0;value[i];i++) if (value[i]=='\"') value[i] = ' ';

    if(!strchr(device, '@')) 
      if (pattern?!regexec(&reexp, device,0,NULL,0):1) {
	if ((end_t==0 || end_t>date) && !strcmp(state,"ActualValue")) {
	  parameter *p = parameter::find(device, 0);
	  if (!start_t) start_t = date;
	  date =atol(linedate);
	  if ((date-start_t)/60.0>0) p->fill((date-start_t)/60.0, atof(value));
	}
      }
  }
