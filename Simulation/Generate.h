//                                                                    -*-c++-*-
// $Id: Generate.h 2689 2017-05-31 10:21:51Z schlimme $
...
class generateDMBoson : public eventGenerator {
		   ...
		   const FourVector &q_out, const double mA)
  {
    ...
    FourVector p_outCM = q_p + p_inCM - q_outCM;
...
class generateElasticPolElectronPolProton : public eventGenerator {
public:
  virtual ~generateElasticPolElectronPolProton() { ; }
  generateElasticPolElectronPolProton(reaction *r, simDetectorBase *Sime, 
			simDetectorBase *Sim1, SIMUL *rundb) { 
    Label      = "d[s]";
    Unit       = "[m]b";
    Reaction   = r; sime=Sime; sim1=Sim1;
...
class generateElasticNucleus : public eventGenerator {
private:
  PseudoRandom prndm;
  int Form;
public:
  virtual ~generateElasticNucleus() { ; }
  generateElasticNucleus(reaction *r, simDetectorBase *Sime, 
			simDetectorBase *Sim1, SIMUL *rundb) { 
    Label      = strdup("d[W]'");
    Unit       = strdup("[m]b");
    Reaction   = r; sime=Sime; sim1=Sim1;
...
class generateQuasiElasticNucleus : public eventGenerator {
  class PWIA *pwia;
private:
  PseudoRandom prndm;

public:
  virtual ~generateQuasiElasticNucleus() { ; }
  generateQuasiElasticNucleus(reaction *r, simDetectorBase *Sime, 
			simDetectorBase *Sim1, SIMUL *rundb) { 
     Label      = strdup("d[W]'");
    Unit       = strdup("[m]b");
    Reaction   = r; sime=Sime; sim1=Sim1;
  ...
  double generateEvent(double helicity);
  double integrationVolume() { return 1; }
};
...
class generateElasticDeuteron : public eventGenerator {
protected:
  FormfactorBase *FF;
public:
  generateElasticDeuteron(reaction *r, simDetectorBase *Sime,
			  simDetectorBase *Sim1, SIMUL *rundb);
  virtual ~generateElasticDeuteron();
  double generateEvent(double); // helicity is ignored
  double generateEvent_V1(); // helicity is ignored
  double generateEvent_V2(); // helicity is ignored
  double generateEvent_V3(); // helicity is ignored
  double generateEvent_VMOD(); // helicity is ignored
  double integrationVolume() { return 4 * sime->getDphi() * dcte; }
};

//new class for nitrogen
class generateElasticNitrogen : public eventGenerator {
public:
  virtual ~generateElasticNitrogen() { ; }
  generateElasticNitrogen(reaction *r, simDetectorBase *Sime, 
			simDetectorBase *Sim1, SIMUL *rundb) { 
    Label      = "d[s]";
    Unit       = "[m]b";
    Reaction   = r; sime=Sime; sim1=Sim1;
...
class generateMott : public eventGenerator {
public:
  virtual ~generateMott() { ; }
   generateMott(reaction *r, simDetectorBase *Sime, 
			simDetectorBase *Sim1, SIMUL *rundb) { 
    Label      = "d[W]'";
    Unit       = "[m]b";
    Reaction   = r; sime=Sime; sim1=Sim1;
...
public:
  generateInclusive(reaction *r, simDetectorBase *Sime,
...
class generateElasticPeakingApprox : public eventGenerator {
public:
  virtual ~generateElasticPeakingApprox() { ; }
  generateElasticPeakingApprox(reaction *r, simDetectorBase *Sime, 
			simDetectorBase *Sim1, SIMUL *rundb) { 
    Label      = "d[s]";
    Unit       = "[m]b";
    Reaction   = r; sime=Sime; sim1=Sim1;
...
class generateElasticHavar : public eventGenerator {
private:
  PseudoRandom prndm;
public:
  virtual ~generateElasticHavar() { ; }
  generateElasticHavar(reaction *r, simDetectorBase *Sime, 
			simDetectorBase *Sim1, SIMUL *rundb) { 
    Label      = "d[s]";
    Unit       = "[m]b";
    Reaction   = r; sime=Sime; sim1=Sim1;
...
class generateElasticCryogens : public eventGenerator {
private:
  PseudoRandom prndm;
public:
  virtual ~generateElasticCryogens() { ; }
  generateElasticCryogens(reaction *r, simDetectorBase *Sime, 
			simDetectorBase *Sim1, SIMUL *rundb) { 
    Label      = "d[s]";
    Unit       = "[m]b";
    Reaction   = r; sime=Sime; sim1=Sim1;
