...
//
// $Header: /tmp/cvsroot/Cola/ColaMIT/rund.cc,v 2.3 2002-12-12 23:52:12 distler Exp $
//
...
printHelp(char * progname)
{
  ...
  std::cerr << "\trun server and storage program" << std::endl << std::endl;
  std::cerr << " -f <file>\t" << "input  file. default: STDIN" << std::endl;
  std::cerr << " -p <port>\t" << "port number. default: 8000" << std::endl;
  std::cerr << " -v\t\t"      << "verbose flag." << std::endl;
  std::cerr << " -z\t\t"      << "gzip file output." << std::endl << std::endl;
