...
//
// $Header: /tmp/cvsroot/Cola/ColaMIT/Q2Cindy.cc,v 2.2 2002-12-12 12:24:12 distler Exp $
//
...
printHelp(char * progname)
{
  ...
  cerr << " -Q <dir>\t"    << "data directory. default: cwd" << endl;
  cerr << " -R\t\t"        << "receive mode." << endl;
  cerr << " -c\t\t"        << "concatenate beam/detector events." << endl;
  cerr << " -d\t\t"        << "debug flag." << endl;
  cerr << " -e <select>\t" << "process events ??, only." << endl;
  ...
  cerr << " -8\t\t"        << "process event 8, only." << endl << endl;
...
//
// Function name : parseMask(const char *maskstr)
//

unsigned int
parseMask(const char *maskstr)
{
  unsigned int mask = 0xffffffff;

  if (!maskstr) return mask;
  int inverse = (maskstr[0] == '!');

  if (inverse) maskstr++;
  while (*maskstr) {
    int event = atoi(maskstr);
    if ((1 <= event) && (event < 32)) mask &= ~(1 << (event-1));
    while (('0' <= *maskstr) && (*maskstr <= '9')) maskstr++;
    if (*maskstr) maskstr++;
  }

  return (inverse ? mask : ~mask);
} // unsigned int parseMask(const char *)


//
// Function name : main(int argc, char *argv[])
//
// Description   : main function - create Qtape object
...
main(int argc, char *argv[])
{
  ...
  while (EOF != (opt = getopt(argc, argv, "Q:Rcde:f:hno:vz8?"))) {
    switch (opt) {
    case 'Q':
      Qdir   = optarg;
      break;
    case 'R':
      ...
      break;
    case 'c':
      OOPSdataRecord::concatBeamEvents = 1;
      break;
    case 'd':
      flags |= debugFlag;
      break;
    case 'e':
      mask = parseMask(optarg);
      break;
    case 'f':
      input  = optarg;
      break;
  ...
  if (input && (*input == ':')) {
    ...
    if (!Qdir) {
      Qdir = new char[MAXPATHLEN];
      getcwd(Qdir, MAXPATHLEN);
    }
    if (access(Qdir, R_OK|W_OK|X_OK)) {
      perror(argv[0]);
      return errno;
    }
    if (chdir(Qdir)) {
  ...
  OOPStape oopsTape(ct, fd, copyrun);
  if (mask != 0xffffffff) oopsTape.setMask(mask);
  while (!gotSignal && (Qtape::EoF != oopsTape.nextRecord())) {
    if (!(flags & specialFlag)) oopsTape.handle();
  }
